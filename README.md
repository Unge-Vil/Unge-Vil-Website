...
Tag a release like `v0.5.3` and GitHub Actions will attach zips for the child theme and each plugin.
...
3. In **Tag version**, enter a new tag like `v0.5.3` (use `MAJOR.MINOR.PATCH` with a `v` prefix).
...
> To tag from the command line instead, run `git tag v0.5.3 && git push origin v0.5.3` before drafting the release.
