...
Tag a release like `0.5.7` and GitHub Actions will attach zips for the child theme and each plugin.
...
3. In **Tag version**, enter a new tag like `0.5.7` (use `MAJOR.MINOR.PATCH`).
...
> To tag from the command line instead, run `git tag 0.5.7 && git push origin 0.5.7` before drafting the release.
