...
Tag a release like `0.7.8` and GitHub Actions will attach zips for the child theme and each plugin.
...
3. In **Tag version**, enter a new tag like `0.7.8` (use `MAJOR.MINOR.PATCH`).
...
> To tag from the command line instead, run `git tag 0.7.8 && git push origin 0.7.8` before drafting the release.
